 ...
 * SSE 流式：AI 边生成边推 chunk，完成后存库推 done 事件
 * data 格式统一为 { type: 'progress'|'chunk'|'done'|'error', ...payload }
 */
export const generateReport = async (req: Request, res: Response): Promise<void> => {
  ...
  const abItems = await prisma.telegraph.findMany({
    where: { ctime: { gte: startOfDay, lt: endOfDay }, level: { in: ['A', 'B'] } },
    orderBy: { ctime: 'desc' },
    take: 60,
  });
  const cItems = await prisma.telegraph.findMany({
    where: { ctime: { gte: startOfDay, lt: endOfDay }, level: 'C' },
    orderBy: { ctime: 'desc' },
    take: 40,
  });
  const items = [...abItems, ...cItems];

  if (!items.length) {
    send('error', { message: '暂无新闻数据' });
    res.end();
    return;
  }

  send('progress', { message: '已加载 ' + items.length + ' 条新闻，AI 分析中...' });
  ...
  const prompt = [
    '【角色】你是拥有20年实战经验的A股顶级操盘手，曾管理百亿私募，擅长从财经新闻中精准捕捉主力资金动向、政策套利机会和板块轮动节奏。你的选股逻辑：新闻催化 + 资金承接 + 技术位配合，三者缺一不可。',
    '',
    '【今日电报 · ' + date + '】',
    headlines,
    constraintText,
    '【任务】基于以上电报，从主力资金视角精选今明两日最具爆发力的A股标的。',
    ...
    '      "logic": "选股核心逻辑：新闻催化+资金面+操作建议，50字内",',
    '      "risk": "主要风险点，20字内",',
    '      "horizon": "短线或中线",',
    '      "strength": "强或中或弱",',
    ...
    '4. strength 判断标准：',
    '   强 = 政策直接点名/业绩大超预期/主力资金明显流入，确定性高',
    '   中 = 间接受益/预期改善/板块联动，需要配合市场情绪',
    '   弱 = 题材炒作/消息面刺激/跟风机会，风险较高',
    '5. logic 必须包含：①新闻中的直接依据 ②为什么这只股票受益 ③建议操作方向',
  ...
  try {
    ...
    const report = await prisma.dailyReport.upsert({
      where: { date },
      create: { date, stocks: JSON.stringify(parsed.stocks ?? []), summary: parsed.summary ?? '', market_mood: parsed.market_mood ?? '' },
      update: { stocks: JSON.stringify(parsed.stocks ?? []), summary: parsed.summary ?? '', market_mood: parsed.market_mood ?? '' },
    });

    send('done', { id: report.id, date: report.date, stocks: report.stocks, summary: report.summary, market_mood: report.market_mood, createdAt: report.createdAt.toISOString() });
    res.end();
  } catch (err: any) {
